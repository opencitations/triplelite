...
By default, LiteGraph builds a single index mapping subjects to their predicates and objects (SPO). Queries that start from a subject are a dictionary lookup. Queries that start from an object scan the entire graph.
...
g = LiteGraph(reverse_index_predicates=frozenset({FRBR_PART_OF, RDF_TYPE}))
...
g = LiteGraph(reverse_index_predicates=frozenset({f"{FRBR}partOf"}))
